...
export const AdvancedUserProfileModal: React.FC<AdvancedUserProfileModalProps> = ({
  ...
  const [theme, setTheme] = useState<WindowThemeConfig>(() => getSecretWindowTheme('user_profile'));
  ...
  useEffect(() => {
    const handleUpdate = (e: Event) => {
      ...
      if (customEvent.detail?.windowId === 'user_profile') {
      ...
      <AnimatePresence>
        ...
        >
          ...
          <motion.div
            ...
            className="w-full max-w-full sm:max-w-md rounded-t-3xl shadow-[0_-10px_35px_rgba(0,0,0,0.25)] relative flex flex-col dir-rtl select-none pb-3 border-t max-h-[60vh] overflow-visible mt-8 transition-all"
            dir="rtl"
          >
            ...
            <div className="flex items-center gap-1.5">
              ...
              {isDevUser && (
                <button
                  ...
                  className="w-6 h-6 rounded-full border border-cyan-400/60 bg-cyan-500/20 flex items-center justify-center text-cyan-300 hover:bg-cyan-500/40 transition-all cursor-pointer shadow-[0_0_8px_rgba(6,182,212,0.5)]"
                  title="تخصيص سري للملف الشخصي (المبرمج فقط)"
                >
              ...
              <button
                ...
                className="w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-slate-300 hover:bg-white/10 transition-all cursor-pointer"
                title="معلومات / بلاغ"
              >
            ...
            {(isCurrentAdmin || isRoomOwner) && (
              <div className="w-full pt-1.5 border-t border-slate-100 mt-1">
                <div className="grid grid-cols-5 gap-0.5 text-center">
                  ...
                  {/* 2. Dedicated Mute/Unmute Mic Control (كتم المايك / إلغاء الكتم للمشرفين والمالك والمستخدم نفسه بكامل الصلاحيات) */}
                  {(() => {
