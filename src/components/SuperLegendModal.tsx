...
export const SuperLegendModal: React.FC<SuperLegendModalProps> = ({ isOpen, onClose, onOpenRecharge }) => {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-md" dir="rtl">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 20 }}
          className="w-full max-w-md h-full sm:h-[90vh] bg-[#0A1618] text-white sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-amber-500/20 relative"
        >
