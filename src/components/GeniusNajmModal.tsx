...
export interface GeniusNajmModalProps {
  isOpen: boolean;
  onClose: () => void;
  userAvatar?: string;
  userName?: string;
...
export type GeniusYoHoModalProps = GeniusNajmModalProps;

export const GeniusNajmModal: React.FC<GeniusNajmModalProps> = ({
  isOpen,
  onClose,
        ...
        <div className="flex-1 overflow-y-auto px-3.5 py-2 space-y-4 pb-28">
          ...
          <div className="space-y-2">
            ...
            {rankingList.map((item, idx) => (
              <motion.div
                key={`genius-rank-${item.rank}-${item.name}-${idx}`}
              ...
              <div className="w-full max-w-sm bg-gradient-to-b from-slate-900 to-slate-950 border border-amber-500/40 rounded-3xl p-5 shadow-2xl space-y-4">
                <div className="flex items-center justify-between border-b border-white/10 pb-3">
                  <h3 className="text-sm font-black text-amber-300 flex items-center gap-2">
                    ...
                    <span>قواعد تصنيف تميز النجم</span>
                  </h3>
                  <button
...
export const GeniusYoHoModal = GeniusNajmModal;
export default GeniusNajmModal;
