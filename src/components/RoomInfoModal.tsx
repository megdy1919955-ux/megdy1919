...
interface RoomInfoModalProps {
  ...
  hostAvatar?: string;
  hostName?: string;
  userRole?: 'owner' | 'moderator' | 'guest';
  onOpenSettings?: () => void;
}
...
export const RoomInfoModal: React.FC<RoomInfoModalProps> = ({
  ...
  // Current active role for testing/viewing mode
  const [currentRole, setCurrentRole] = useState<'owner' | 'moderator' | 'guest'>(userRole);
  ...
  const [adminsList, setAdminsList] = useState<AdminUser[]>([
    ...
    },
    {
      ...
      role: 'admin',
      roleTitle: 'مشرف أول - إدارة الروم',
      level: 'Lv.75',
      vip: 'VIP6',
      nLevel: 'N.12',
      isOnline: true,
    ...
    },
    {
      ...
      role: 'moderator',
      roleTitle: 'مشرف المايكات والحضور',
      level: 'Lv.64',
      vip: 'VIP5',
      nLevel: 'N.10',
      isOnline: true,
    ...
    },
    {
      ...
      role: 'moderator',
      roleTitle: 'مدير الموسيقى والدي جي',
      level: 'Lv.52',
      vip: 'VIP4',
      nLevel: 'N.8',
      isOnline: false,
      ...
      <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        >
          ...
          <div className="space-y-3 shrink-0">
            {/* Simulation Role Selector Pill */}
            <div className="bg-[#1A2234] p-1.5 rounded-2xl border border-white/10 flex items-center justify-between text-xs">
              <span className="text-[10px] font-black text-slate-400 flex items-center gap-1">
                <Eye className="w-3 h-3 text-cyan-400" />
                <span>معاينة الصلاحيات:</span>
              </span>
            ...
            <div className="grid grid-cols-2 gap-1 bg-[#1A2234] p-1 rounded-2xl border border-white/10 text-center">
              <button
              ...
              >
                ...
                <span>{isOwner ? 'مسؤولو الغرفة والصلاحيات' : 'قائمة المشرفين والمسؤولين'}</span>
              </button>
              ...
              >
                ...
                <span>إعلان ومعلومات الغرفة</span>
              </button>
            </div>
          </div>
          ...
          <div className="overflow-y-auto no-scrollbar my-3 space-y-2.5 flex-1 min-h-[260px] pr-0.5">
            {/* TAB 1: ADMINS & MODERATORS LIST */}
            {activeTab === 'admins' && (
              <div className="space-y-2.5">
                <div className="flex items-center justify-between px-1">
                  <span className="text-xs font-black text-amber-300 flex items-center gap-1">
                    <Crown className="w-4 h-4 text-amber-400" />
                    <span>{isOwner ? 'إدارة طاقم المشرفين وتوزيع الصلاحيات' : 'قائمة مشرفي الغرفة'}</span>
                  </span>
                ...
                <div className="space-y-2">
                  {adminsList.map((admin) => {
                    const isRoomOwner = admin.role === 'owner';
            ...
            {/* TAB 2: ROOM ANNOUNCEMENT & INFO */}
            {activeTab === 'announcement' && (
              <div className="space-y-3">
                {/* Dedicated Room Announcement Card */}
                <div className="bg-gradient-to-r from-[#1A2234] via-slate-900 to-purple-950/60 border border-purple-500/40 rounded-2xl p-3.5 space-y-2.5 shadow-lg relative">
                  <div className="flex items-center justify-between">
                  ...
                  {isOwner && isEditingAnnouncement ? (
                    ...
                    <div className="bg-slate-950/60 p-3 rounded-xl border border-white/5 text-slate-200 text-xs leading-relaxed font-bold dir-rtl">
                      {announcement}
                    </div>
                  )}
                ...
                {/* Advanced Room Settings Button (Owner Only) */}
                {isOwner && (
