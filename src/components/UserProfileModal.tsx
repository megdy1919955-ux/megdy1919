...
import { X, Edit3, Copy, ChevronLeft, Car, Gift, Camera, Plus, Heart, Trash2 } from 'lucide-react';
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs rounded-xl flex items-center gap-1 shadow-md transition-all cursor-pointer"
                >
              ...
              {moments.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
                  ...
                  <button
                    onClick={() => momentInputRef.current?.click()}
                    className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center gap-1.5"
                  >
                    <Plus className="w-4 h-4 stroke-[3]" />
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {/* زر + الزائد الدائم لاختيار ونشر صور من استديو الهاتف */}
                  <div
                    onClick={() => momentInputRef.current?.click()}
                    className="relative aspect-square bg-slate-900/80 hover:bg-slate-900 border-2 border-dashed border-amber-500/60 hover:border-amber-400 rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all group shadow-sm text-center p-2"
                  >
                    <div className="w-10 h-10 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center group-hover:scale-110 group-hover:bg-amber-500 group-hover:text-slate-950 transition-all">
                      <Plus className="w-6 h-6 stroke-[3]" />
                    </div>
                    <span className="text-xs font-black text-amber-300 mt-2">إضافة صورة من الاستديو</span>
                  </div>

                  {moments.map((item) => (
                    <div key={item.id} className="relative aspect-square rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 shadow-lg group">
                      ...
                      <button
                        ...
                        className="absolute top-2 left-2 p-1.5 bg-rose-600/90 hover:bg-rose-500 text-white rounded-full shadow-md transition-all cursor-pointer opacity-80 hover:opacity-100 z-10"
                        title="حذف اللحظة"
                      >
                      ...
                      <div className="absolute bottom-2 right-2 left-2 flex items-center justify-between text-white text-[10px]">
                        <span className="font-bold text-slate-300">{item.time}</span>
                        <span className="flex items-center gap-1 font-mono font-black text-rose-400 bg-black/40 px-1.5 py-0.5 rounded-md">
